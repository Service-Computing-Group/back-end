...
func planetsHandler(w http.ResponseWriter, req *http.Request) {
	w.WriteHeader(http.StatusOK)
	w.Header().Set("Access-Control-Allow-Origin", "*")
	vals := req.URL.Query()
	page := 1

	itemCount := database.GetCount("planets")
	...
	for i := 1; count < pagelen*page; i++ {
		...
		if len(item) != 0 {
			...
			if count > 10*(page-1) {
				w.Write([]byte(item))
				if count >= pagelen*page || count >= database.GetCount("planets") {
					break
				}
				w.Write([]byte(", \n"))
			}
...
func getPlanetById(w http.ResponseWriter, req *http.Request) {
	vars := mux.Vars(req)
